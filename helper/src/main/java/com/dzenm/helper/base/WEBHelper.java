...
/**
 * @author dzenm
 * @date 2020/5/3 下午5:06
 */
public class WEBHelper {

    private AppCompatActivity mActivity;
    private WebView mWebView;
    private String mCurrentTitle, mCurrentUrl;

    public WEBHelper(AppCompatActivity activity) {
        mActivity = activity;
    }
     ...
     * @param url         初次加载的url
     */
    public void enabledWebView(
            @NonNull ViewGroup viewGroup, @Nullable ProgressBar progressBar, String url
    ) {
     ...
     * @param url       初次加载的url
     */
    private void showWebView(@NonNull ViewGroup viewGroup, String url) {
        // 避免WebView内存泄露。不在xml中定义 WebView ，而是在需要的时候在Activity中创建，并且Context使用 getApplicationContext()
        mActivity.getSupportActionBar().setTitle("正在加载中...");
...
//        webView.loadData(String data,String mimeType, String encoding);
    ...
    private void setWebClient(
            @NonNull final WebView webView, @Nullable final ProgressBar progressBar
    ) {
...
//            <uses-permission android:name="android.permission.ACCESS_COARSE_LOCATION" />
            ...
            public boolean onJsConfirm(WebView view, String url, String message, final JsResult result) {
                new AlertDialog.Builder(mActivity)
                        ...
                        .setMessage(message)
                        .setPositiveButton(mActivity.getText(R.string.dialog_btn_confirm), new DialogInterface.OnClickListener() {
                            @Override
     ...
     */
    @SuppressLint("SetJavaScriptEnabled")
    private void setWebSettings(@NonNull WebSettings webSettings) {
        // JS相关设置
...
//        webSettings.setUserAgentString("Mozilla/5.0 (Windows NT 10.0; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0");
    ...
    protected void shareCurrentPage(String prompt) {
        ShareHelper.newInstance(mActivity)
    ...
    protected void copyCurrentPageUrl() {
        ScreenHelper.copy(mActivity, mWebView.getUrl());
    ...
    public void onBackPressed() {
        if (mWebView != null) {
            if (mWebView.copyBackForwardList().getCurrentIndex() > 0) {        // 判断当前历史列表是否
                goBack();
                if (!mWebView.canGoBack()) mActivity.onBackPressed();
            } else {
                mActivity.finish();
            }
        }
